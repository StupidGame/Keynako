...
- 複数の辞書語と未変換の助詞をつないだ文全体の組み合わせ候補
- Windowsではコマンドプロンプトなど新しい入力先でIMEが起動した直後は英数入力。必要なときに入力メニューや`Ctrl+Space`で日本語へ切り替え
- 読みが前方一致する共有辞書・標準辞書の予測候補
- `「`、`(`などの開き記号を入力したときの閉じ記号補完と内側へのカーソル移動
- `A`表示時の英語直接入力
- 入力ごとに未確定文字列と先頭候補を更新するライブ変換
- `Space`／変換キーで変換開始と次候補、上下／Page Up／Page Downによる候補選択。未入力時の変換キーはモード切替に使わない
...
- Android/iOS IMEでも削除キーを約0.35秒以内で2回押すと単語削除。カスタムタブ下部の削除キーも同じ動作。周辺文字列を公開しない入力欄では通常の一文字削除へ戻す
- Custardの後方`smart_delete`と`smartDeleteDefault`は単語境界を使って削除。おぐら配列形式の`delete` + `smart_delete`も1回の単語削除として処理
- `Ctrl+Space`による日本語／英語モード切替。未確定入力と変換状態を保ったまま切替後の候補を作り直す
...
配布用`v`タグのActions成果物に含まれる`KeynakoSetup.exe`を実行します。セットアップは管理者権限で設定アプリ、64-bit TSF DLL、ZenzaiとモデルをProgram Filesへ配置し、Keynakoを日本語入力プロファイルとして登録します。Windowsの入力設定からKeynakoを選択できます。Pull Requestと手動実行の成果物は署名なしの検査用です。

選択中はWindowsの入力インジケーターへ、Flutterアプリと同じブランドアイコンと、現在の入力モードを表す`あ`／`A`を個別に表示します。モード項目をクリックすると日本語と英語を切り替えられ、メニューからライブ変換、共有辞書の手動更新、設定アプリを操作できます。角丸の候補ウィンドウには選択行、候補番号、ページ数、キー操作、共有辞書／Zenzaiの出典を表示します。候補を右クリックすると、その単語と読みをアプリの辞書登録と同じ共有ストレージへ送信します。第一候補以外の辞書候補を確定すると8秒間だけ改善の送信確認を表示し、クリックした場合に限って選択語、読み、候補順位、アプリ版番号をHTTPSで送ります。入力の前後や入力先アプリの情報は送りません。共有辞書の定期取得は設定アプリが行い、IMEは入力中にローカルキャッシュだけを再読込します。キー入力による非表示プロセスの自動起動は行いません。

ゲームや全画面アプリがIME候補を独自表示する場合は、TSFのUIなし候補一覧を通して候補、選択位置、確定操作をアプリ側へ渡します。

Windows版は`WH_KEYBOARD`などのキーボードフックを使わず、TSFのキーイベントと予約キーだけで入力します。配布用`v`タグではKeynako製DLL／exeとインストーラーを信頼されたAuthenticode証明書で署名します。ゲーム側が独自の許可リストを使う場合は署名済みでも個別対応が必要になるため、すべてのアンチチート製品での受理を保証するものではありません。
