...
- フリック入力、ローマ字入力、入力欄ごとに標準・カスタムタブ・Custard配列を選べる自動切り替え、記号・絵文字・顔文字入力
...
Shift／Caps Lockを別タブで実装する配列でも、英語（`en_US`）のタブ間を`move_tab`で移動する際は未確定文字列を維持します。大文字入力後に通常タブへ戻る複数アクションも同様です。日本語や直接入力のタブへ移るときは確定します。
...
「着せ替えを作成／編集」の背景画像から端末内の画像を選ぶと、最大2048px・8MB以下に調整したコピーをアプリ専用領域へ保存し、AndroidとiOSのキーボード背景へ反映します。Androidの片手モードでは背景画像を画面幅へ固定し、キー面だけを左右へ寄せるため、キーのない側にも着せ替えが続きます。背景画像があるときはキー背景の透過度を0〜85%から選べます。画像ファイルは端末内だけに保存され、テーマJSONの共有対象には含まれません。
